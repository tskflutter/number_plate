...
class AddNewDepositController extends GetxController {
  ...
  Future<void> submitDeposit() async {
    ...
    ResponseModel responseModel = await depositRepo.insertDeposit(isBoost: isBoost, adsId: adId, methodCode: paymentMethod?.methodCode ?? "", currency: paymentMethod?.currency ?? "");
    ...
    if (responseModel.statusCode == 200) {
      ...
      if (insertResponseModel.status.toString().toLowerCase() == "success") {
        showWebView(insertResponseModel.data?.redirectUrl ?? "", isBoost);
      } else {
  ...
  void showWebView(dynamic redirectUrl, bool isBoostAd) {
    String url = '';

    if (redirectUrl is List && redirectUrl.isNotEmpty) {
      url = redirectUrl.first.toString();
    } else {
